...
## Exact OT problem
Kantorovich's optimal transport (OT) problem search for a deterministic transportation plan $\pi^\*(x_1 | x_0)$ that satisfy 
...
The entropic regularized OT (EOT) problem relaxes the deterministic plan with a stochastic map $\pi^\*(x_1 | x_0)$, and the OT problem can be viewed as the special case of $\delta$ distribution 

$\pi^* = \inf_{\pi \in \Pi(q_0, q_1)} \int \frac{1}{2} ||x_1 - x_0 ||^2 d\pi(x_0, x_1) - \sigma^2 H(\pi) = \sigma^2 D(\pi | Q_{0, 1})$
...
$H(\pi) = \int_{\mathcal{X}} \ln \pi d\pi$ is the entropic term, and $D(\pi | Q_{0, 1})$ denotes the distance measure of the mapping given the marginal condition $Q_{0, 1}$. 
...
$\inf_{\rho_t, u_t} \int_\mathcal{X} \int_{0}^{1} \frac{1}{2} ||u_t(x)||^2 d\rho_t(x) dt$
...
$u^* = \mathop{\mathrm{arg\,inf}}\_{u} KL(P^u | Q), \quad dQ_t = \sigma dW_t, \quad Q_0 = q_0$

By Girsanov's theorem {sarkkaAppliedStochasticDifferential2019} 

$KL(P^u | Q) = \frac{1}{\sigma^2} \mathbb{E}\_{P^u}\left[ \int_{0}^{1} \frac{1}{2} \lVert u_t(X_t^u) \rVert^2 dt \right]$
...
$u^* = \inf_u \int_{0}^{1} \int_{\mathcal{X}} \frac{1}{2} ||u_t(x)||^2 d\rho_t(x) dt$

s.t.

$\partial_t \rho_t + \nabla \cdot (u_t \rho_t) - \frac{1}{2} \sigma^2 \Delta \rho_t = 0, \quad \rho_0 = q_0, \quad \rho_1 = q_1$

The Reciprocal Property {leonardReciprocalProcessesMeasuretheoretical2014}chenStochasticControlLiaisons2021}{shiDiffusionSchrodingerBridge2023} of SB indicates the minimization objective $KL(P^u | Q)$ can be decomposed to be 

$KL(P^u | Q) = D(P_{0, 1}^u | Q_{0, 1}) + \int_{\mathcal{X} \times \mathcal{X}} \int_{0}^{1} KL(P_t^u(\cdot | x_0, x_1) | Q_t(\cdot | x_0, x_1)) dt dP_{0, 1}(x_0, x_1)$
...
$P_t^*(\cdot | x_0, x_1) = Q_t(\cdot | x_0, x_1), \quad (x_0, x_1) \sim P^*_{0, 1}-a.s.$

In other words, the marginal-conditioned measure of optimal path $P_t^u$ is a Brownian bridge 

$P_t^*(\cdot | x_0, x_1) = Q_t(\cdot | x_0, x_1) = \mathcal{N}(\cdot | (1-t)x_0 + tx_1, \sigma^2 t(1-t)I)$

And the SB problem is reduced to 

$KL(P^* | Q) = D(P_{0, 1}^* | Q_{0, 1})$
